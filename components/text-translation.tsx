...
export default function TextTranslation({
  ...
  const handleTranslate = async () => {
    ...
    try {
        ...
        if (!recaptchaToken && !isDisabled) {
          ...
          if (process.env.NODE_ENV === "development") {
            toast({
              ...
              description: "Continuing without security verification in development mode",
              variant: "destructive",
            })
          }
        }
      ...
      console.log("Translation raw response:", responseText.substring(0, 100) + "...")
    ...
    try {
      ...
      if (data.language && data.language !== sourceLanguage) {
        ...
        toast({
          ...
          description: `Detected language: ${detectedLanguage?.name || data.language}`,
        })
      } else {
        toast({
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to detect language. Please try again.",
        variant: "destructive",
      })
    } finally {
  ...
  return (
    <div className="space-y-4 mt-4">
      ...
      <div className="flex justify-center">
        <Button onClick={handleTranslate} disabled={!inputText.trim() || isTranslating} className="w-full sm:w-auto">
          {isTranslating ? (
            <>
      ...
      {translatedText && (
        <div className="space-y-2">
          <div className="relative">
            <Textarea value={translatedText} readOnly className="min-h-[120px]" />
            <div className="absolute top-2 right-2 flex space-x-1">
              <Button variant="ghost" size="icon" onClick={handleCopy} title="Copy to clipboard">
                <Copy className="h-4 w-4" />
                <span className="sr-only">Copy to clipboard</span>
              </Button>
              <Button variant="ghost" size="icon" onClick={handleSave} title="Save to history">
