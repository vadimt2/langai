...
export default function DocumentTranslation({
      ...
      if (
        ...
        try {
          ...
          toast({
            ...
            description:
              'Failed to load PDF processing library. Please try again later.',
            variant: 'destructive',
          });
        }
  ...
  useEffect(() => {
    const handlePaste = (e: ClipboardEvent) => {
      ...
      if (e.clipboardData && e.clipboardData.items) {
        ...
        // Loop through the items to find a file
        for (let i = 0; i < items.length; i++) {
          // Check if it's a file and it's one of our supported types
          const supportedTypes = Object.keys(SUPPORTED_FILE_TYPES);
          if (
            items[i].kind === 'file' &&
            (items[i].type === 'text/plain' ||
              items[i].type === 'application/pdf' ||
              items[i].type.includes('document') ||
              items[i].type.includes('spreadsheet'))
          ) {
      ...
      if (!isPdfJsLoaded) {
        if (!isPdfJsLoading) {
          ...
          toast({
            ...
            description:
              'Please wait while we load the PDF processing library...',
          });
        } else {
          ...
          toast({
            ...
            description:
              'PDF processing library is loading. Your file will be processed automatically when ready.',
          });
        }
        return;
      }
    ...
    try {
      ...
      try {
        ...
        if (!extractedText.trim()) {
          throw new Error(
            'No text could be extracted from the PDF. The PDF might be scanned or contain only images.'
          );
        }
        ...
        toast({
          ...
          description: `Successfully extracted text from ${numPages} page${
            numPages !== 1 ? 's' : ''
          }. Translating now...`,
        });
  ...
  const extractFileContent = async (file: File) => {
    ...
    try {
      ...
      if (file.type === 'text/plain') {
        try {
          ...
          toast({
            ...
            description:
              'Successfully extracted text from file. Translating now...',
          });
      ...
      toast({
        title: 'Text Extracted',
        description:
          'Successfully extracted text from document. Translating now...',
      });
  ...
  const handleTranslate = async (contentToTranslate?: string) => {
    ...
    try {
      ...
      const processBatch = async (startIndex: number) => {
        ...
        for (
          let i = startIndex;
          i < Math.min(startIndex + MAX_CONCURRENT_REQUESTS, chunks.length);
          i++
        ) {
          batchPromises.push(
            (async (index) => {
        ...
        if (
          startIndex + MAX_CONCURRENT_REQUESTS < chunks.length &&
          !signal.aborted
        ) {
  ...
  const handleSave = () => {
    if (fileContent && translatedText) {
      addToHistory({
        ...
        targetLanguage,
        sourceText: file
          ? `[Document: ${file.name}] ${fileContent.substring(0, 100)}...`
          : fileContent,
  ...
  const smartSplitTextIntoChunks = (
    text: string,
    chunkSize: number
  ): string[] => {
  ...
  const splitLargeTextIntoChunks = (
    text: string,
    chunkSize: number
  ): string[] => {
    const chunks: string[] = [];
              ...
              >
                ...
                <p className='text-sm text-gray-500 mb-2'>
                  {isDragging
                    ...
                    : 'Drag & drop, click, or paste (Ctrl+V) to upload a document'}
                </p>
                <p className='text-xs text-gray-400 mb-4'>
                  Supported formats: TXT, PDF (best support), Word (DOCX), Excel
                  (XLSX)
                </p>
                {isPdfJsLoading && (
            ...
            <div className='space-y-4'>
              ...
              {pendingPdfFile && (
                <Alert>
                  ...
                  <AlertDescription>
                    Please wait while we load the PDF processing library. Your
                    file will be processed automatically when ready.
                  </AlertDescription>
                </Alert>
              )}
                ...
                <div className='space-y-2'>
                  <div className='flex items-center justify-between'>
                    <div className='flex items-center'>
                      ...
                      <p>
                        {isExtracting ? 'Extracting text...' : 'Translating...'}
                      </p>
                    </div>
